  ...
  return (
    <div className="w-full h-full flex flex-col p-6 bg-white/80 backdrop-blur-sm">
      ...
      <div className="flex-grow overflow-y-auto mt-4 space-y-6">
        ...
        {/* File Manager */}
        <div className="bg-lime-50 p-4 rounded-lg shadow-inner">
            <h2 className="text-xl font-bold text-yellow-900 mb-2">{t('downloads')}</h2>
            <p className="text-gray-600 mb-4">{t('downloadsDescription')}</p>
            <button 
                onClick={() => { playSound('CLICK'); setScreen(Screen.DOWNLOADS); }}
                className="w-full bg-blue-500 text-white font-bold p-3 rounded-lg shadow-md hover:bg-blue-600 transition-colors flex items-center justify-center gap-2">
                <DownloadCloudIcon />
                <span>{t('manageFiles')}</span>
            </button>
        </div>
